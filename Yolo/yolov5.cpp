...
//    }
    ...
    static float data[BATCH_SIZE * 3 * 480 * 640];//三通道输入
...
//                    std::cout<<data[ 3 * INPUT_H * INPUT_W + i]<<std::endl;
    ...
    std::vector<targetPose> targetPose_ori;
    std::vector<targetPose> targetPose_double;
    if(double_flag == 10)
    {
        std::sort(result_sets.begin(), result_sets.end(),my_compare);// sort done
    }
    else
    {
        if(double_flag == 0)
        {
            ...
            {
                if(result_sets.at(i).class_ == 0||result_sets.at(i).class_ == 2)
                {
                    targetPose targetPose_temp;
                    targetPose_temp.result_ = result_sets.at(i);
                    targetPose_temp.distance = pow(result_sets.at(i).box_.x+result_sets.at(i).box_.width/2-320,2)+pow(result_sets.at(i).box_.y+result_sets.at(i).box_.height/2-480,2);
                    targetPose_temp.angle = 180.0f/3.1415926*atan2(result_sets.at(i).box_.x+result_sets.at(i).box_.width/2-320,480-result_sets.at(i).box_.y+result_sets.at(i).box_.height/2);
                    targetPose_double.emplace_back(targetPose_temp);
                }
                else
                {
                    targetPose targetPose_temp;
                    ...
                    targetPose_temp.distance = pow(result_sets.at(i).box_.x+result_sets.at(i).box_.width/2-320,2)+pow(result_sets.at(i).box_.y+result_sets.at(i).box_.height/2-480,2);
                    targetPose_temp.angle = 180.0f/3.1415926*atan2(result_sets.at(i).box_.x+result_sets.at(i).box_.width/2-320,480-result_sets.at(i).box_.y+result_sets.at(i).box_.height/2);
                    targetPose_ori.emplace_back(targetPose_temp);
                }
            }
        }
        ...
        else
        {
            ...
            for(int i=0;i<result_sets.size();i++)
            {
                if(result_sets.at(i).class_ == double_flag)
                {
                    targetPose targetPose_temp;
                    targetPose_temp.result_ = result_sets.at(i);
                    targetPose_temp.distance = pow(result_sets.at(i).box_.x+result_sets.at(i).box_.width/2-320,2)+pow(result_sets.at(i).box_.y+result_sets.at(i).box_.height/2-480,2);
   ...
   // std::cout<<targetPose_double.at(0).result_.class_<<std::endl;
    cv::Point2f tar_Pos;
    result result_temp;
    if(!targetPose_double.empty()&&!targetPose_ori.empty())
    {
        for(int i=0;i<targetPose_double.size();i++)
        {
            int status = 0;
            for(int j=0;j<targetPose_ori.size();j++)
            {
                if(targetPose_ori.at(j).distance>targetPose_double.at(i).distance)
                {
                    status++;
                    continue;
                }
                ...
                {
			/*
                    if(abs(targetPose_ori.at(j).angle - targetPose_double.at(i).angle) > 18)
                    {
                        status++;
                        continue;
		    }
		    ...
		    */
            ...
            if(status == targetPose_ori.size())
            {
                targetPose_double.at(i).status = true;
            }
        }
        std::sort(targetPose_double.begin(), targetPose_double.end(),my_compare2);// sort done

        bool double_status = false;
	for(int i =0;i<targetPose_double.size();i++)
        {
            if(targetPose_double.at(i).status == true)
            {
                class_ = targetPose_double.at(i).result_.class_;
                tar_Pos.x = targetPose_double.at(i).result_.box_.x+targetPose_double.at(i).result_.box_.width/2;
                tar_Pos.y = targetPose_double.at(i).result_.box_.y+targetPose_double.at(i).result_.box_.height;
          result_temp.box_ = targetPose_double.at(i).result_.box_;
        ...
        if(double_status == false)
        {
        std::sort(result_sets.begin(), result_sets.end(),my_compare);// sort done
        class_ = result_sets.at(0).class_;
        tar_Pos.x = result_sets.at(0).box_.x+result_sets.at(0).box_.width/2;
        tar_Pos.y = result_sets.at(0).box_.y+result_sets.at(0).box_.height;
        result_temp.box_ = result_sets.at(0).box_;
       	}
    }
    else if(!targetPose_double.empty()&&targetPose_ori.empty())
    {
        std::sort(targetPose_double.begin(), targetPose_double.end(),my_compare2);// sort done


        class_ = targetPose_double.at(0).result_.class_;

        tar_Pos.x = targetPose_double.at(0).result_.box_.x+targetPose_double.at(0).result_.box_.width/2;
        tar_Pos.y = targetPose_double.at(0).result_.box_.y+targetPose_double.at(0).result_.box_.height;
    result_temp.box_ = targetPose_double.at(0).result_.box_;
...
//            }
  ...
  /*  for(int i =0;i<targetPose_double.size();i++)
    {
        std::cout<<"targetPose_double.at(i) "<<"angle: "<<targetPose_double.at(i).angle<<"distance: "<<targetPose_double.at(i).distance<<"status: "<<targetPose_double.at(i).status<<std::endl;
    }

    for(int i =0;i<targetPose_ori.size();i++)
    {
        std::cout<<"targetPose_ori.at(i) "<<"angle: "<<targetPose_ori.at(i).angle<<"distance: "<<targetPose_ori.at(i).distance<<std::endl;
    }
*/
...
//	     std::cout<<"class:"<<class_<<std::endl;
            ...
            cv::rectangle(src, result_temp.box_, cv::Scalar(0, 0, 255), 2);
